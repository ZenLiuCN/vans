...
const osc = /^\x1b]8;[\x20-\x3a\x3c-\x7e]*;([\x21-\x7e]{0,512})(?:(?:\x1b\\)|(?:\x07))([\x20-\x7e]+)\x1b]8;;(?:(?:\x1b\\)|(?:\x07))/
const osc_st = /(?:(\x1b)|(\x07))|([\x00-\x06]|[\x08-\x1a]|[\x1c-\x1f])/g
const esc = "\x1b"
...
class ANSI {
    ...
    toHtml(escape,txt, tag, link,span, line) {
        if (!txt) return tag([])
        ...
        while (i >= 0) {
            esc:
                switch (true) {
                    case i > 0:
                        if(url){
                            ch.push(...this.parseOSC(url, x.substring(0, i), link))
                            url = ''
                        }else{
        ...
        if (x) {
           if(url){
               ch.push(...this.parseOSC(url, x,link))
           }else{
