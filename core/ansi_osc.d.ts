...
type LineRender<T> = ()=>T
type LinkRender<T> = (url:string|undefined,text:string)=>T
type Render<T> = ( c:T[])=>T
...
interface ANSI{
    ...
    toHtml<T>(escape:boolean,txt: string, tag: Render<T>,link:LinkRender<T>,span:SpanRender<T>,line:LineRender<T>)
}
